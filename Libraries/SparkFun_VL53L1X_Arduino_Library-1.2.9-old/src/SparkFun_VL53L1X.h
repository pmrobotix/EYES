...
class SFEVL53L1X
{
	...
	* To set the center, set the pad that is to the right and above the exact center of the region you'd like to measure as your opticalCenter*/
	...
	void setROI(uint8_t x, uint8_t y); //Set the height and width of the ROI in SPADs, lowest possible option is 4.
	void setROICenter(uint8_t x, uint8_t y, uint8_t center);
          void setCenter( uint8_t center);
          /**
          ...
          void getResult(VL53L1X_Result_t *result); // Gets the measurement and ranging status in a single read
